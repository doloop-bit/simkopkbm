<script>
    ...
    let { 
        children, 
        footer,
        programName = 'Academic Elegance', 
        programLogo = null,
        registrationLink = '/pendaftaran',
        registrationLabel = 'Daftar',
        navLinks = [
            { name: 'Home', href: '/' },
    ...
    <nav class="fixed top-0 w-full z-50 transition-all duration-300 font-headline tracking-tight {isScrolled ? 'bg-white/95 dark:bg-slate-900/95 py-3 shadow-md backdrop-blur-lg' : 'bg-transparent py-5'}">
        <div class="max-w-7xl mx-auto px-6 md:px-12 flex justify-between items-center">
            ...
            <div class="flex items-center gap-4">
                <Link href={registrationLink} class="hidden sm:block elegant-btn-primary !px-6 !py-2.5 !text-sm">
                    {registrationLabel}
                </Link>
                <button 
    ...
    {#if mobileMenuOpen}
        ...
        >
            ...
            {#each navLinks as link}
                ...
                {/if}
            {/each}
            <Link href={registrationLink} class="elegant-btn-primary w-full max-w-xs mt-8 py-4 text-xl" onclick={() => mobileMenuOpen = false}>
                {registrationLabel} Sekarang
            </Link>
        </div>
    {/if}
        ...
        <footer class="bg-[#00210c] dark:bg-slate-950 text-white pt-20 pb-10 font-body leading-relaxed border-t-8 border-elegant-accent mt-20">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 px-6 md:px-12 max-w-7xl mx-auto">
                <div class="space-y-6">
                    ...
                    <p class="text-emerald-100/70 text-sm">Mendedikasikan diri untuk masa depan pendidikan yang berlandaskan iman, inovasi, and karakter mulia.</p>
                     <div class="flex gap-4">
                        <Link class="w-10 h-10 rounded-full border border-elegant-accent/30 flex items-center justify-center hover:bg-elegant-accent transition-colors" href="/" aria-label="Bagikan Halaman">
                            <span class="material-symbols-outlined text-sm">share</span>
                        </Link>
                        <Link class="w-10 h-10 rounded-full border border-elegant-accent/30 flex items-center justify-center hover:bg-elegant-accent transition-colors" href="/" aria-label="Situs Publik">
                ...
                <div>
                    <h4 class="font-headline font-bold mb-6 text-elegant-accent">Tautan Langsung</h4>
                     <ul class="space-y-4 text-sm text-emerald-100/80">
                        <li><Link class="hover:text-elegant-accent transition-colors duration-200" href="/tentang-kami">Tentang Kami</Link></li>
                        <li><Link class="hover:text-elegant-accent transition-colors duration-200" href="/program-pendidikan">Program Akademik</Link></li>
                        <li><Link class="hover:text-elegant-accent transition-colors duration-200" href="/pendaftaran">Pendaftaran Online</Link></li>
                        <li><Link class="hover:text-elegant-accent transition-colors duration-200" href="/program-pendidikan/tahfizh">Program Tahfizh</Link></li>
                    </ul>
                </div>
                ...
                <div>
                    <h4 class="font-headline font-bold mb-6 text-elegant-accent">Informasi</h4>
                     <ul class="space-y-4 text-sm text-emerald-100/80">
                        <li><Link class="hover:text-elegant-accent transition-colors duration-200" href="/">Privacy Policy</Link></li>
                        <li><Link class="hover:text-elegant-accent transition-colors duration-200" href="/">Terms of Service</Link></li>
                        <li><Link class="hover:text-elegant-accent transition-colors duration-200" href="/">Campus Map</Link></li>
                        <li><Link class="hover:text-elegant-accent transition-colors duration-200" href="/">Adab & Policy</Link></li>
                    </ul>
                </div>
