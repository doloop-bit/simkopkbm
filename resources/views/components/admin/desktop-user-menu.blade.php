<div x-data="{ open: false }" class="relative w-full">
    <button @click="open = !open" @click.outside="open = false" type="button" class="flex items-center gap-3 w-full px-2 py-2 rounded-xl hover:bg-slate-800/50 transition-colors cursor-pointer">
        <div class="flex-1 min-w-0 text-left">
            <div class="text-sm font-semibold text-slate-200 truncate">{{ auth()->user()->name }}</div>
            <div class="text-xs text-slate-400 truncate">{{ auth()->user()->email }}</div>
        </div>
        <x-ui.icon name="o-chevron-up-down" class="w-4 h-4 text-slate-500 shrink-0" />
    </button>
    ...
    <div x-show="open" x-cloak
         ...
         @click="open = false"
         class="absolute bottom-full left-0 right-0 mb-2 rounded-xl border border-slate-700 bg-slate-800 shadow-xl py-1 z-50">
