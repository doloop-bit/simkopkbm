...
<body class="min-h-screen bg-gradient-mesh font-sans antialiased text-slate-900 dark:text-slate-100 selection:bg-emerald-500/30"
    ...
    <div class="flex min-h-screen relative">
        ...
        <aside
            ...
            class="fixed inset-y-0 left-0 z-50 sidebar-premium transition-all duration-300 lg:translate-x-0 flex flex-col h-screen bg-base-200 lg:bg-transparent"
        >
            {{-- Fixed Logo Header --}}
            <div class="px-5 py-6 shrink-0 relative group h-[88px]">
                <div class="flex items-center justify-between h-full">
                    <a href="{{ route('teacher.dashboard') }}" class="flex items-center gap-3 no-underline min-w-0 transition-opacity" :class="sidebarCollapsed ? 'opacity-100 group-hover:opacity-0 justify-center w-full' : ''" wire:navigate>
            ...
            <div class="flex-1 overflow-y-auto custom-scrollbar px-2">
                <x-teacher.sidebar :only-menu="true" />
            </div>
        ...
        <div :class="sidebarCollapsed ? 'lg:ml-20' : 'lg:ml-64'" class="flex-1 min-w-0 transition-all duration-300">
            <div class="min-h-screen flex flex-col">
                ...
                <div class="p-responsive flex-1">
                    @php
                        $hasSubNav = request()->routeIs('teacher.report-cards') || request()->routeIs('teacher.assessments.grading') || request()->routeIs('teacher.assessments.attendance') || request()->routeIs('teacher.assessments.extracurricular');
                    @endphp

                    @if ($hasSubNav)
                        {{-- Sticky pill nav for teacher tools --}}
