        ...
        <x-ui.card padding="false">
            <div class="p-5 pb-3 flex items-center justify-between">
                <div>
                    ...
                    <p class="text-xs text-slate-400 mt-0.5">{{ __('10 siswa dengan tunggakan tertinggi') }}</p>
                </div>
            </div>
                ...
                <table class="w-full text-sm text-left text-slate-600 dark:text-slate-400">
                    <thead class="text-xs text-slate-500 uppercase bg-slate-50 dark:bg-slate-800/50">
                        <tr>
                            ...
                            <th class="px-5 py-3">{{ __('Jenjang') }}</th>
                            <th class="px-5 py-3 text-center">{{ __('Jumlah Tagihan') }}</th>
                            <th class="px-5 py-3 text-right">{{ __('Total Tunggakan') }}</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                        @foreach($chartData['topDebtors'] as $debtor)
                            <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-800/30">
                                <td class="px-5 py-3 font-semibold text-slate-900 dark:text-white">{{ $debtor['name'] }}</td>
                                <td class="px-5 py-3">{{ $debtor['level'] }}</td>
                                <td class="px-5 py-3 text-center">{{ $debtor['billing_count'] }}</td>
                                <td class="px-5 py-3 text-right font-mono font-bold text-rose-600 dark:text-rose-400">
                                    Rp {{ number_format($debtor['unpaid'], 0, ',', '.') }}
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
