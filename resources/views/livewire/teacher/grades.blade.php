...
new #[Layout('components.teacher.layouts.app')] class extends Component {
    ...
    public function loadScores(): void
    {
        ...
        // Verify teacher has access to this classroom and subject
        $teacher = auth()->user();
        if (!$teacher->hasAccessToClassroom($this->classroom_id) || !$teacher->hasAccessToSubject($this->subject_id)) {
            $this->scores_data = [];
            \Flux::toast(variant: 'danger', text: 'Anda tidak memiliki akses ke kelas atau mata pelajaran ini.');
            return;
        }
    ...
    public function save(): void
    {
        ...
        $teacher = auth()->user();
        if (!$teacher->hasAccessToClassroom($this->classroom_id) || !$teacher->hasAccessToSubject($this->subject_id)) {
            \Flux::toast(variant: 'danger', text: 'Anda tidak memiliki akses untuk menyimpan nilai ini.');
            return;
        }
    ...
    public function with(): array
    {
        ...
        $assignedSubjectIds = $teacher->getAssignedSubjectIds();
...
<div class="p-6">
    <div class="flex items-center justify-between mb-6">
        <div>
            ...
            <flux:subheading>Input nilai siswa untuk mata pelajaran yang Anda ampu.</flux:subheading>
        </div>
    </div>
